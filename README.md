...
- 柱子的位置? 柱子的数量? 柱子的表示法（以空挡的位置表示？）x坐标表示最左侧位置，y坐标表示空档上端位置，空档长度固定
...
while (true) {
    switch (state) {
        ...
        case 1: 
            ...
            if (pipe1_x < BIRD_X || pipe2_x < BIRD_X || pipe3_x < BIRD_X) {
                score++;
            }
...
bool check_collision(int y_bird, int pipe_x, int pipe_y) {
    // 柱子是否到了鸟的位置
    if (pipe_x < BIRD_X && pipe_x > BIRD_X-PIPE_WIDTH) {
        // 检查鸟是否与柱子上下边界发生碰撞
        if (y_bird < pipe_y || y_bird > (pipe_y + PIPE_GAP)) {
            return true;
        }
    }
    return false; // 无碰撞
