...
def generate_mif_content(data_list, width, comments=None):
    ...
    address_width = math.ceil(math.log2(actual_depth))
    ...
    if actual_depth < required_depth:
        if required_depth - actual_depth > 1:
            mif_lines.append(f"[{actual_depth}..{required_depth - 1}] : 0;")
        else:
            mif_lines.append(f"{actual_depth} : 0;")
...
def process_images(input_folder, output_folder):
            ...
            for y in range(height):
                for x in range(width):
                    ...
                    if pixels[x, y][3] == 0xFF:
                        # Only consider non-transparent colors
    ...
    color_map = [0] * (len(all_colors) + 1) # [0] is hardcoded to represent transparent color
    for key, value in all_colors.items():
        color_map[value] = key
    color_map_path = os.path.join(output_folder, "color_map.mif")
    with open(color_map_path, "w") as f:
        mif_content = generate_mif_content(color_map, width=24, comments=["Color Map"])
        f.write(mif_content)

    all_colors_bits = math.ceil(math.log2(len(color_map)))
    for filename, pixel_map in image_pixel_maps.items():
        ...
        with open(output_path, "w") as f:
            mif_content = generate_mif_content(
                pixel_map,
                width=all_colors_bits,
                comments=[f"Image: {filename}", f"Size: {width}x{height} pixels"],
            )
            f.write(mif_content)
...
if __name__ == "__main__":
    total_colors = process_images("./reduced", "./")
